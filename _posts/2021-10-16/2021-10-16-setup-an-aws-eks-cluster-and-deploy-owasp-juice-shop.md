---
canonical_url: "https://dev.to/aws-builders/aws-eks-setup-cluster-and-deploy-owasp-juice-shop-214j"
date: "2021-10-16"
title: "Setup an AWS EKS cluster and deploy OWASP juice shop"
tags: "kubernetes, aws, owasp, eks"
categories: "kubernetes, aws, owasp, eks"
---
**This post first appeared on [dev.to](https://dev.to/aws-builders/aws-eks-setup-cluster-and-deploy-owasp-juice-shop-214j)**

Let's setup an [EKS cluster](Launch EKS cluster
https://youtu.be/rJQb_whepEY) and then install the 
...
![Add IAM User](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/17caiqd55l6viemvpczu.png)
 

On the next page, you can create a group, if there isn't one. ![Add IAM Group](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/1dooy08ej1iq3h4nh8fv.png)
 

I am going to create a group for the Administrators with the AdministratorAccess policy.
![Administrator Access](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/7ndd22u6bloxt0zlgrat.png)
 


The user can now be mapped to the Administrators group. ![Map Group](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/v3zzblwjqj08zdd6ooys.png)
 

I would be skipping the tags stage and finally create the user.![Create User](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/sjxca5b8folppw5vckg7.png)
...
We just need to create the credentials file in the .aws directory, and store the access key id and secret access key retrieved earlier. So that it looks like the following, note that the file below has dummy values. Check this [link](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html) for reference.
```
┌─[nc@parrot]─[~]
└──╼ $cat .aws/credentials 
[default]
...
## EKSCTL
So we have done the essential settings for the AWS CLI. Let's now continue with [eksctl](https://docs.aws.amazon.com/eks/latest/userguide/getting-started-eksctl.html) installation.
```
┌─[nc@parrot]─[~]
...
For more cluster customizations, you may visit the [EKS getting started guide](https://docs.aws.amazon.com/eks/latest/userguide/getting-started.html).

## Kubeconfig
Let's check the list of clusters using [aws eks cli](https://docs.aws.amazon.com/cli/latest/reference/eks/index.html). We need to specify the region or configure the default region prior to listing. In my case its us-west-2.
```
┌─[✗]─[nc@parrot]─[~]
...
![OWASP Juice Shop](https://dev-to-uploads.s3.amazonaws.com/uploads/articles/nyq89kbe4qo0ms0sjtkp.png)

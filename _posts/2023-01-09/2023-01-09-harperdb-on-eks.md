...
tags: "security, webdev, showdev"
categories: "security, webdev, showdev"
---
**This post first appeared on [dev.to](https://dev.to/aws-builders/harperdb-on-eks-1bcb)**

Hi there :wave:, let's see how to deploy HarperDB on EKS, and then test it with an API call from CURL. You can get the Kubernetes manifests that we make in this post from this [link](https://github.com/networkandcode/harperdb-deployments/tree/main/kubernetes-manifests).

Hope you are already familiar with topics such as [Deployment](https://youtu.be/udSPJXMU9L4?t=1094), [Load Balancer](https://www.youtube.com/watch?v=7o6koGCsoSc) service, [Secret](https://www.youtube.com/watch?v=iTkz1bIacHM) and [Persistent volume claim](https://www.youtube.com/watch?v=vqk0Ccd7sio)
...
## Cluster
We can now create an EKS cluster with eksctl. You may see this [video](https://www.youtube.com/watch?v=rJQb_whepEY) for cluster creation from the CLI.
```
...
## Docker hub
We can visit the docker [hub](https://hub.docker.com/r/harperdb/harperdb) page of harperdb to get an idea on the ports, environment variables, volume path etc.
...
Please follow this [link](https://aws.amazon.com/premiumsupport/knowledge-center/eks-persistent-storage/) to add IAM role in AWS cloud, and ebs csi objects on the cluster. This should fix the PVC issue.
...
Send a CURL command to test schema creation. The endpoint is from the external IP column in the service. You may check this [video](https://www.youtube.com/watch?v=25L8VvXgx8w) to know how to obtain the curl command for harperdb.
```

...
This post builds on top of [this](https://dev.to/aws-builders/harperdb-on-eks-1bcb) where we discussed how to launch HarperDB on EKS with kubectl. Here, we would use those same manifests. But, we would go with the helm cli tool instead of kubectl for the deployment. Please check this [link](https://github.com/networkandcode/harperdb-deployments/tree/main/helm-charts/minimal/harperdb) for the final chart we make in this post.

Get set go :fire:

First, ensure [helm](https://dev.to/aws-builders/harperdb-on-eks-1bcb) is installed. If you want to use AWS cloud shell for this, please click on the shell icon on the top navigation bar of the cloud console, and execute the following.
```
